...
char* get_4digitnum(){   // 4 xonali tasodifiy sonni chiqarib beruvchi funksiya
    char* digitnum = malloc(4);   // xotiradan 4ta belgi sig`adigan 4 bayt joy ajratadi
    int dig = 0;
    ...
    while(strlen(digitnum) < 4){     //kodning uzunligin tekshirish
        ...
        if(in_inner(digitnum, m))
            digitnum[dig++] = m;     // 23-line -> 8-line   ---> o`yin shartiga ko`ra, tasodifiy 4xonali sonda 1ta raqam 2marta ishtirok etmasligi kerak, buni tekshirish uchun yuqorida bool toifasidagi funksiya ochib olamiz
    }
    return digitnum;
}
...
bool in_revision(char* str){      // matn ichida 1ta raqam 2 marta ishtirok etganini tekshirish
...
bool wrong_input(char* text){       // inputni harf yoki harf emasligini tekshirish
    ...
    int i = 0;
    while(i < strlen(text)){
        if(text[i] > '7' || text[i] < '0' || in_revision(text))
            return true;
        i++;
    }
    return false;
